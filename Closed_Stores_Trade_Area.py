...
Instore_Transaction_data_full = pd.read_pickle("C:\\HALO Effect\\BBBY_closed_Instore_Transactions_v2.pkl")

Distance_data_full = pd.read_pickle("C:\\HALO Effect\\BBBY_closed_store_Distance_data_full.pkl")

BOPIS_full = pd.read_pickle("C:\\HALO Effect\\BBBY_closed_store_BOPIS_v2.pkl")
...
store_params = pd.read_csv("C:\\HALO Effect\\closure_best_params_stores.csv")
...
#    clusters[['Home_Store_counts']] = Home_Store_Scaler.inverse_transform(clusters[['Home_Store_counts_scaled']])
    ...
    clusters_final_v2[['FIPS_STATE_CD','FIPS_COUNTY_CD','CENSUS_BLOCK_CD','GEOID','Store Number']].drop_duplicates().to_csv("C:\\HALO Effect\\Closed_Stores_BBBY_Results_BOPIS_Phase_1_no_HS\\"+str(jobs2000)+"_clusters.csv", index=False)
...
if __name__ == "__main__": 
    ...
    QUERY = """select distinct Store__ from `dw-bq-data-d00.QUANT_STG.sk_store_closings` where Concept = 'BBBY' and extract(year from Close_Date) = 2020 and extract(month from Close_Date) = 12;"""
    open_stores = bq_client.query(QUERY, job_config=job_config).result().to_dataframe()
#    print(open_stores.head())

    store_list = list(open_stores['Store__'].unique()) 
