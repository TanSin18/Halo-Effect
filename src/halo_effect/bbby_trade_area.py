"""
BBBY Trade Area Analysis (Open Stores)

This module performs trade area identification for open Bed Bath & Beyond stores
...
from sklearn.cluster import DBSCAN, KMeans  # Fixed: Added KMeans import
...
def load_data():
    ...
    data_files = {
        'transactions': config.BBBY_TRANSACTION_DATA,
        'distance': config.BBBY_DISTANCE_DATA,
        'bopis': config.BBBY_BOPIS_DATA,
        'best_params': config.BBBY_BEST_PARAMS,
    }
    ...
    # Load data
    data = {
        'transactions': pd.read_pickle(str(config.BBBY_TRANSACTION_DATA)),
        'distance': pd.read_pickle(str(config.BBBY_DISTANCE_DATA)),
        'bopis': pd.read_pickle(str(config.BBBY_BOPIS_DATA)),
        'best_params': pd.read_csv(str(config.BBBY_BEST_PARAMS)),
    }
...
def clustering_func(store_number, data, census_block_group_v2):
    """
    Perform single-phase DBSCAN clustering with KMeans refinement for a single store
    ...
    try:
        ...
        output_file = config.CLUSTERS_OUTPUT_DIR / f"bbby_{store_number}_clusters.csv"
        clusters_final_v2[[
            'FIPS_STATE_CD', 'FIPS_COUNTY_CD', 'CENSUS_BLOCK_CD',
            'GEOID', 'Store Number'
...
def main():
    ...
    print("=" * 60)
    print("BBBY Open Stores Trade Area Analysis - Starting")
    print("=" * 60)
